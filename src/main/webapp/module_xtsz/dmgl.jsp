...
<script language="javascript">
	...
	function addDM() {
		$('#dlg').dialog('open').dialog('setTitle','添加断面信息');
		$('#ff').form('clear');
		
		url = 'AddSectionsAction';
	}
	
	function editDM() {
		var row = $('#tt').datagrid('getSelected');
		if (row){
			$('#dlg').dialog('open').dialog('setTitle','修改断面信息');
			$('#ff').form('load',row);
			$('#Name').val(row.name);
			$('#Des').val(row.des);
			url = 'EditSectionsAction?Id='+row.id;
		} else {
			alert('没有选择要修改的断面！');
		}
	}
	...
	function delDM() {
		var row = $('#tt').datagrid('getSelected');
		if (row){
			$.messager.confirm('系统提示','确定要删除所选断面?',function(r){
				if (r){
					$.post('DelSectionsAction',{Id:row.id},function(result){
						if (result.success){
							$('#tt').datagrid('reload');	// reload the user data
						} else {
...
<body>
		<table id="tt" class="easyui-datagrid" style="width:100%;" pagination="true" toolbar="#tb"
			url="GetSectionsAction" rownumbers="true" singleSelect="true" pageSize="20">
        <thead>
            <tr>
	...
	<form id="fm" method="post">
		<div id="tb" style="padding:3px">
			...
			<div style="border-top:1px solid #b8d0d6;padding:0px;">
				<a href="javascript:void(0)" onclick="addDM();" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'">添加</a>
				<a href="javascript:void(0)" onclick="editDM();" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-edit'">修改</a>
				<a href="javascript:void(0)" onclick="delDM();" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-remove'">删除</a>
			</div>
		</div>
	</form>
	
	<div id="dlg-buttons">
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="saveDM()"  style="width:80px">保存</a>
       <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#dlg').dialog('close')"
			style="width:80px;">取消</a>
    </div>
	<div id="dlg" class="easyui-dialog" title="断面参数设置" data-options="closed:true,buttons:'#dlg-buttons'" 
