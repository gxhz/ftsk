...
<script language="javascript">
	...
	function addSYQCD() {
		$('#dlg').dialog('open').dialog('setTitle','添加大坝测点信息');
		$('#ff').form('clear');
		
		url = 'AddDamMonitoringpointsAction';
	}
	
	function editSYQCD() {
		var row = $('#tt').datagrid('getSelected');
		if (row){
			$('#dlg').dialog('open').dialog('setTitle','修改大坝测点信息');
			$('#ff').form('load',row);
			$('#Name').val(row.name);
			$('#Sbid').combobox('setValue', row.device_id);
			$('#Skid').combobox('setValue', row.sections_id);
			url = 'EditDamMonitoringpointsAction?Id='+row.id;
		} else {
			alert('没有选择要修改的大坝测点！');
		}
	}
	...
	function delSYQCD() {
		var row = $('#tt').datagrid('getSelected');
		if (row){
			$.messager.confirm('系统提示','确定要删除所选大坝测点?',function(r){
				if (r){
					$.post('DelDamMonitoringpointsAction',{Id:row.id},function(result){
						if (result.success){
							$('#tt').datagrid('reload');	// reload the user data
						} else {
...
<body>
		<table id="tt" class="easyui-datagrid" style="width:100%;" pagination="true" toolbar="#tb"
			url="GetDamMonitoringpointsAction" rownumbers="true" singleSelect="true" pageSize="20">
        <thead>
            <tr>
                <th field="name" width="400">监测点名称</th>
                <th field="device_name" width="300">所属设备</th>
                <th field="sections_name" width="300">所属断面</th>
                <th data-options="field:'device_id',sortable:true,hidden:true">所属设备ID</th>
                <th data-options="field:'sections_id',sortable:true,hidden:true">所属断面ID</th>
            </tr>
        </thead>
		</table>
	...
	<div id="dlg" class="easyui-dialog" title="大坝测点参数设置" data-options="closed:true,buttons:'#dlg-buttons'" 
