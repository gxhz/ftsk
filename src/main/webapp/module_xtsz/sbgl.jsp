...
<script language="javascript">
	...
	function editSB() {
		...
		if (row){
			...
			$('#Name').val(row.name);
			$('#Des').val(row.des);
			url = 'EditDevicesAction?Id='+row.id;
		} else {
			alert('没有选择要修改的设备！');
		}
	}
	...
	function delSB() {
		var row = $('#tt').datagrid('getSelected');
		if (row){
			$.messager.confirm('系统提示','确定要删除所选设备?',function(r){
				if (r){
					$.post('DelDevicesAction',{Id:row.id},function(result){
						if (result.success){
							$('#tt').datagrid('reload');	// reload the user data
						} else {
...
<body>
		<table id="tt" class="easyui-datagrid" style="width:100%;" pagination="true" toolbar="#tb"
			url="GetDevicesAction" rownumbers="true" singleSelect="true" pageSize="20">
        <thead>
            <tr>
              <th field="code" width="120">设备ID</th>
	...
	<form id="fm" method="post">
		<div id="tb" style="padding:3px">
			<div style="border:0px solid #b8d0d6;padding:8px;width:width:100%">
				<table class="jgdstyle">
					<tr>
						<td width="50">设备ID：</td>							
						<td width="150">
							<input type="text" name="code" id="code" style="width:120px;">
						</td>
                        <td width="50">名称：</td>                         
                        <td width="150">
			...
			<div style="border-top:1px solid #b8d0d6;padding:0px;">
				<a href="javascript:void(0)" onclick="addSB();" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'">添加</a>
				<a href="javascript:void(0)" onclick="editSB();" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-edit'">修改</a>
				<a href="javascript:void(0)" onclick="delSB();" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-remove'">删除</a>
			</div>
		</div>
	</form>
	
	<div id="dlg-buttons">
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="saveSB()"  style="width:80px">保存</a>
       <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#dlg').dialog('close')"
			style="width:80px;">取消</a>
    </div>
	<div id="dlg" class="easyui-dialog" title="设备参数设置" data-options="closed:true,buttons:'#dlg-buttons'" 
			...
			<div style="padding:5px 0 10px 0;height:250px">
				<form id="ff" method="post">
					<table border="0" cellpadding="0" cellspacing="0" style="width:100%;">
                        <tr>
                            <td height="30px" width="70px">设备ID:</td>
                            <td><input class="easyui-validatebox" id="Code" name="Code" required="true" style="width:99%"></input></td>
                        </tr>
						<tr>
