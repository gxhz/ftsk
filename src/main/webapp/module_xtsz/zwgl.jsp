...
<script language="javascript">
	...
	function editZw() {
		...
		if (row){
			$('#dlg').dialog('open').dialog('setTitle','修改职务信息');
			$('#ff').form('load',row);
			$('#Name').val(row.name);
			$('#Des').val(row.des);
			$('#Unuseflag').combobox('setValue', row.unuseflag);
			url = 'EditJobAction?Id='+row.id;
		} else {
			alert('没有选择要修改的职务！');
		}
	}
	...
	function delZw() {
		var row = $('#tt').datagrid('getSelected');
		if (row){
			$.messager.confirm('系统提示','确定要删除所选职务?',function(r){
				if (r){
					$.post('DelJobAction',{Id:row.id},function(result){
						if (result.success){
							$('#tt').datagrid('reload');	// reload the user data
						} else {
...
<body>
		<table id="tt" class="easyui-datagrid" style="width:100%;" pagination="true" toolbar="#tb"
			url="GetJobAction" rownumbers="true" singleSelect="true" pageSize="20">
        <thead>
            <tr>
              <th field="name" width="260">名称</th>
              <th field="des" width="460">描述</th>
			  <th field="unuseflag" width="200">禁用状态</th>
            </tr>
        </thead>
		</table>
	...
	<form id="fm" method="post">
		<div id="tb" style="padding:3px">
			...
			<div style="border-top:1px solid #b8d0d6;padding:0px;">
				<a href="javascript:void(0)" onclick="addZw();" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'">添加</a>
				<a href="javascript:void(0)" onclick="editZw();" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-edit'">修改</a>
	...
	<div id="dlg-buttons">
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="saveZw()"  style="width:80px">保存</a>
       <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#dlg').dialog('close')"
			style="width:80px;">取消</a>
    </div>
	<div id="dlg" class="easyui-dialog" title="职务设置" data-options="closed:true,buttons:'#dlg-buttons'" 
...
<script language="javascript">
    ...
    function resizeCanvas(){
        var oh=parent.parent.document.body.offsetHeight-210;
